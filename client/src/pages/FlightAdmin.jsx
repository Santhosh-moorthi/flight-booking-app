...
const FlightAdmin = () => {
  ...
  const fetchUserData = async () =>{
    try{
      ...
      await axios.get(`http://localhost:3001/fetch-user/${id}`).then(
        (response)=>{
  ...
  const fetchData = async () =>{
    await axios.get('http://localhost:3001/fetch-bookings').then(
      (response)=>{
        setbookingCount(response.data.filter(booking => booking.flightName === localStorage.getItem('username')).length);
      }
    );
    await axios.get('http://localhost:3001/fetch-flights').then(
      (response)=>{
