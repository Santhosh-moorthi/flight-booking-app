...
const Admin = () => {
  ...
  const fetchData = async () =>{
    await axios.get('http://localhost:3000/fetch-users').then(
      (response)=>{
    ...
    await axios.get('http://localhost:3000/fetch-bookings').then(
      (response)=>{
        setbookingCount(response.data.length);
      }
    );
    await axios.get('http://localhost:3000/fetch-flights').then(
      (response)=>{
  ...
  const approveRequest = async (id) =>{
      try{
          ...
          await axios.post('http://localhost:3000/approve-operator', {id}).then(
            (response)=>{
  ...
  const rejectRequest = async (id) =>{
    try{
      ...
      await axios.post('http://localhost:3000/reject-operator', {id}).then(
        (response)=>{
